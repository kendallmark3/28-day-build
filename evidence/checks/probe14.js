...
h('probe14',async c=>{
  ...
  // A10 corrupt store (owned by Day 19: recorded, not fixed today)
  await c.start('{not json');await page.click('#submitBtn');
  const after10=await c.stored();
  row('A10','stored data is not valid JSON, then the user saves','the app says the data was unreadable and keeps a copy (Day 19)',false,'saved over it silently; stored intents='+(after10&&after10.intents.length)+' (deferred to Day 19)');
  // A11 bad hashes
  ...
  fs.writeFileSync((process.argv[2]||'/tmp/shotX.png'),JSON.stringify(rows,null,1));
});
