require('./h.js')('day20',async c=>{
  ...
  rec('O3a links: Intent to #intents, Context to #references, Evidence and Review to #review, Capability to #capabilities, Build to #progress',JSON.stringify(v.stages.map(s=>s.href))==='["#intents","#references","#progress","#review","#review","#capabilities"]',JSON.stringify(v.stages.map(s=>s.href)));
  const opened=[];
  for(const id of ['intent','context','build','evidence','review','capability']){await c.view('overview');await page.click('#flow li[data-stage="'+id+'"] a');await c.wait(150);opened.push((await page.evaluate(()=>['intents','review','capabilities','overview','references','about'].filter(n=>!document.getElementById('view-'+n).hidden))).join());}
  rec('O3b each link opens its view (Intents, References, Progress, Review, Review, Capabilities)',JSON.stringify(opened)==='["intents","references","overview","review","review","capabilities"]',JSON.stringify(opened));
  // ---- live update
  ...
  const got=[];for(let i=0;i<30&&got.length<6;i++){await page.keyboard.press('Tab');const r=await page.evaluate(()=>{const e=document.activeElement;const s=getComputedStyle(e);return {t:e.closest('#flow')?e.textContent:'',ol:s.outlineStyle!=='none'&&parseFloat(s.outlineWidth)>0};});if(r.t)got.push(r);}
  rec('O5b the six stage links are reachable by keyboard in order (Intents, References, Progress, Review, Review, Capabilities), each with a visible outline',got.length===6&&got.map(g=>g.t).join()==='Go to Intents,Go to References,Go to Progress,Go to Review,Go to Review,Go to Capabilities'&&got.every(g=>g.ol),JSON.stringify(got.map(g=>g.t)));
  // layout
